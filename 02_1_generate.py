...
#
#   token id
#     ↓ embedding table 조회  (lookup: vocab_size × hidden_size 행렬)
...
for step in range(config["max_new_tokens"]):
    ...
    # embedding_table은 (vocab_size=128256, hidden_size=4096) 크기의 가중치 행렬.
    # input_ids의 각 token ID를 인덱스로 삼아 해당 행을 조회(lookup)한다.
    # 예) token ID 42 → embedding_table[42] → 4096차원 벡터 1개
    embedding_table = model.model.embed_tokens.weight          # (128256, 4096)
    input_embeddings = embedding_table[input_ids]              # (1, seq_len, 4096)
    ...
    # lm_head_weight: (128256, 4096) — 전치(T)해서 (4096, 128256)과 행렬곱
    last_hidden    = hidden_states[:, -1, :]                          # (1, 4096)
    lm_head_weight = model.lm_head.weight                             # (128256, 4096)
    logits         = torch.matmul(last_hidden, lm_head_weight.T)      # (1, 128256)
